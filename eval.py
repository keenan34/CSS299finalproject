...
def run_queries(
        queries_filename: str, qp: query_process.QueryProcess, num_results: int = 10
) -> Dict[int, List[str]]:
    """
    Runs queries from queries_filename through the search defined by a query process.
    :param queries_filename: jsonl file with queries.
    :param qp: QueryProcess used to run search.
    ...
    for query_id, query_string in query_id_to_query.items():
        query: Query = qp.query_parser.parse_query(query_string, num_results)
        results: SearchResults = qp.index.search(query)
